	...
	public void run() {
		...
		try {
			...
			do {
				...
				switch (option) {
				case 1:// Register
					toggle = 0; // init toggle - toggle is used to determine whether the user is registering as agent or club
					...
					if (toggle == 1 && db.containsAgent(id, name) == -1) { // toggle - 1: new agent registration
						Agent agent = new Agent(id, name, email); 																	
						db.addAgent(agent);
			...
			if (userAgent != null) { // if - successfully logged in as agent
				do {
					...
					case 1: // Add Player
						...
						player.setAgentID(userAgent.getId()); // set the player's agent ID to the agent logged in who is
																// adding the player?
						send("\t\t\tValuation: ");
						...
						db.addPlayer(player);
						break;
					case 2: // Update Player Valuation
						send("\t\t\tPlayer ID: ");
						id = Integer.parseInt((String) in.readObject());
						index = db.containsPlayer(id); // gets the index of the player in the players ArrayList with a given id
						send("\t\t\tNew Valuation: ");
				...
				do {
					...
					option = Integer.parseInt((String) in.readObject());
					switch (option) {
					case 1: // Search Players by position
						int position = 0;
